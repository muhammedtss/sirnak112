...
export default function YenidoganAlgoritmalarListesi() {
  const [searchTerm, setSearchTerm] = useState("");
  const all = (Object.values(yenidoganData) as any[]).sort((a, b) =>
  ...
  return (
    <PageShell>
      <AppHeader title="Yenidoğan Algoritmaları" back="/algoritmalar" badge={all.length} />
      <div className="sticky top-[57px] z-10 px-4 py-3 glass border-b" style={{ borderColor: "var(--glass-border)" }}>
        <div className="relative max-w-md mx-auto">
          ...
          <input type="text" className="glass-input w-full pl-10 pr-4 py-2.5 text-sm" placeholder="Algoritma ara..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
        </div>
      </div>
      <main className="flex-1 px-4 py-4 w-full max-w-md mx-auto">
        <div className="flex flex-col gap-2.5">
          {filtered.length > 0 ? filtered.map((algo: any) => (
            <Link key={algo.id} href={`/algoritmalar/yenidogan/${algo.id}`} className="glass-card glass-hover flex items-center justify-between px-4 py-3.5 group">
