...
export default function CocukAlgoritmalarListesi() {
  ...
  return (
    <PageShell>
      <AppHeader title="Çocuk Algoritmaları" back="/algoritmalar" badge={all.length} />
      ...
      <main className="flex-1 px-4 py-4 w-full max-w-md mx-auto">
        <div className="flex flex-col gap-2.5">
          {filtered.length > 0 ? filtered.map((algo: any) => (
            <Link key={algo.id} href={`/algoritmalar/cocuk/${algo.id}`} className="glass-card glass-hover flex items-center justify-between px-4 py-3.5 group">
