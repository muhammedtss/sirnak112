...
const sections = [
  ...
  {
    title: "Sözel Yanıt (V) — Pediatrik",
    options: [
      { score: 5, label: "Yaşa uygun kelimeler / cümleler, oryante" },
      { score: 4, label: "Ağlıyor ama teselli edilebilir" },
      { score: 3, label: "Uygunsuz ağlama / bağırma" },
  ...
  },
  {
    title: "Motor Yanıt (M)",
    options: [
      { score: 6, label: "Spontan / emirlere uyar" },
...
export default function GlasgowPediatriPage() {
  ...
  return (
    <PageShell>
      <AppHeader title="Pediatri Glasgow Koma Skalası" back="/skalalar" icon={<ArrowLeft style={{ width: 16, height: 16 }} />} />
      ...
      <main className="flex-1 px-3 sm:px-6 py-4 sm:py-6 w-full max-w-xl mx-auto space-y-4">
        {sections.map((section, si) => (
          <div key={si} className="glass-card rounded-xl border border-white/10  overflow-hidden">
            <div className="bg-green-600 text-white px-4 py-2.5 text-sm font-bold">{section.title}</div>
            <div className="divide-y divide-white/10">
              {section.options.map((opt) => (
                ...
                >
                  <span className={`text-sm font-semibold ${selections[si] === opt.score ? "text-green-400" : "text-white/90"}`}>
                    {opt.label}
                  </span>
                  <span className={`text-xs font-bold px-2.5 py-1 rounded-full ${
                    selections[si] === opt.score ? "bg-green-500/15 text-green-400 border border-green-500/30" : "bg-white/5 text-white/50 border border-white/10"
                  }`}>
                    {opt.score}
                  </span>
                </button>
